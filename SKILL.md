...
- coloured entry bars and inline highlights with `<color="blue">...</color>`; the value accepts configured `gray`, `pink`, `blue`, or a direct `#RRGGBB` / `R,G,B` value
...
The named `<color="gray|pink|blue">` backgrounds are configurable through `theme.colors`:
