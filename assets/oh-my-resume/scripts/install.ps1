...
if (-not $xelatex.Ok) {
  Write-Host ''
  Write-Host 'Install MiKTeX Basic, preferably outside C: if disk space is tight: https://miktex.org/download' -ForegroundColor Yellow
}
if (-not $perl.Ok) {
  Write-Host ''
  Write-Host 'Install Strawberry Perl so MiKTeX latexmk can run: https://strawberryperl.com/' -ForegroundColor Yellow
}
if (-not $latexmk.Ok) {
  Write-Host ''
  Write-Host 'Install the MiKTeX latexmk package, then reopen the terminal if needed.' -ForegroundColor Yellow
}
